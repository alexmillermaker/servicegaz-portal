...
6. Completed and confirmed: prepare a separate numbered reference map of first-floor transition nodes without changing the working navigation screen or route graph. F1-S01 maps to F2-S01, F1-S02 maps to F2-S02, and F1-B01 is the confirmed first-floor passage from АБК to Производственный корпус 1.
7. Current temporary implementation, explicitly authorized by the user: use a clearly labelled first-floor Производственный корпус 1 placeholder with B2-E01 linked to F1-B01 and one test destination B2-P01. Use it to validate segmented cross-floor/cross-building routing. Replace only the placeholder map, coordinates, nodes, and edges after the user supplies the real Производственный корпус 1 plan; do not treat temporary geometry as fact.
...
9. Completed and deployed: inter-building transitions must be explicitly marked; the confirmed F1-B01 to B2-E01 transition is marked, and the АБК floor 1 changing-room/toilet/shower block keeps the `Вход через производственный корпус 1` access note.
...
11. Current implementation: the general production overview uses the user's marked `Общий план.svg` derivative at `public/maps/site_plan.svg`. Technical overview object mapping is fixed in `docs/navigation/site-plan-object-index.md`; do not return to the temporary drawn overview map.
12. Current implementation: smart zoom on the general production overview shows a soft prompt before opening the selected site object and shows a soft prompt back to the general overview when zooming out from an object map/state. Current confirmed detailed smart-zoom maps are АБК and Производственный корпус 1; all other site objects open the standard “map not loaded yet” state until real plans are supplied.
